...
这里c是初始化为空字符，空字符在ASCII中的值是0，所以用%d形式输出是0



数组指针：

`int (*p)[3]`中圆括号化先级比方括号高，所以p先与*结合变成指针，后面方括号的3表示它指向的一个有三个元素的数组。
